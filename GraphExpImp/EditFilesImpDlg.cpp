// EditFilesImpDlg.cpp : implementation file
//
...
EditFilesImpDlg::EditFilesImpDlg(CWnd* pParent /*=NULL*/)
	: CDialog(EditFilesImpDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(EditFilesImpDlg)
	m_File_name_str = _T("");
	m_Windows_int = -1;
...
void EditFilesImpDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(EditFilesImpDlg)
...
BEGIN_MESSAGE_MAP(EditFilesImpDlg, CDialog)
	//{{AFX_MSG_MAP(EditFilesImpDlg)
...
/////////////////////////////////////////////////////////////////////////////
// EditFilesImpDlg message handlers

BOOL EditFilesImpDlg::OnInitDialog()
{
	...
	string s;
	ifstream in;
	CString FileName, S, Type;
	CWnd* pWnd;
	int i = 0, start = 0, end = 0;
	char buffer[MAX_PATH];
	bool Pline = false, Text = false, Point = false, CurrentSelect = false;
	TplineRead rstl;
	...
	for (int j = 0; j < m_Accepting_type_lines.GetCount(); j++)
	{
		m_Accepting_type_lines.SetCurSel(j);
		m_Accepting_type_lines.GetWindowText(buffer, MAX_PATH);
		if (!rstl.IsBOF())
			rstl.MoveFirst();
		while (!rstl.IsEOF())
		{
			if (rstl.m_TYPBLINE == AcceptingTypeLines)
			{
				Type = rstl.m_NAMTP;
				break;
			}
	...
	if (!CurrentSelect)
		m_Accepting_type_lines.SetCurSel(0);
	CurrentSelect = false;
	for (j = 0; j < m_Accepting_type_text.GetCount(); j++)
	{
		m_Accepting_type_text.SetCurSel(j);
		m_Accepting_type_text.GetWindowText(buffer, MAX_PATH);
		if (!rstn.IsBOF())
			rstn.MoveFirst();
		while (!rstn.IsEOF())
		{
			if (rstn.m_TYPNAM == AcceptingTypeText)
			{
				Type = rstn.m_NAMNAM;
				break;
			}
	...
	if (TransformDenotations <= 0)
	{
		...
		CurrentSelect = false;
		for (j = 0; j < m_Type_transformed_denotations.GetCount(); j++)
		{
			m_Type_transformed_denotations.SetCurSel(j);
			m_Type_transformed_denotations.GetWindowText(buffer, MAX_PATH);
			if (!rstl.IsBOF())
				rstl.MoveFirst();
			while (!rstl.IsEOF())
			{
				if (rstl.m_TYPBLINE == TypeTransformedDenotations)
				{
					Type = rstl.m_NAMTP;
					break;
				}
	...
	else
	{
		...
		CButton* PrimitiveButton = (CButton*)this->GetDlgItem(IDC_PRIMITIVES);
		PrimitiveButton->SetCheck(1);
		m_Type_transformed_denotations.SetCurSel(0);
	}
	if (m_Import_charakteristics.GetCheck())
	{
		pWnd = GetDlgItem(IDC_SELECT_NAME_CHARAKTERISTICS);
	...
	CurrentSelect = false;
	for (j = 0; j < m_Accepting_type_charakteristics.GetCount(); j++)
	{
		m_Accepting_type_charakteristics.SetCurSel(j);
		m_Accepting_type_charakteristics.GetWindowText(buffer, MAX_PATH);
		if (!rstn.IsBOF())
			rstn.MoveFirst();
		while (!rstn.IsEOF())
		{
			if (rstn.m_TYPNAM == AcceptingTypeCharakteristics)
			{
				Type = rstn.m_NAMNAM;
				break;
			}
	...
	if (!CurrentSelect)
		m_Accepting_type_charakteristics.SetCurSel(0);
	in.open(Folder + "\\" + m_File_name_str + ".mif");
	if (in.is_open())
	{
	...
	CurrentSelect = false;
	for (j = 0; j < m_Name_field_charakteristics.GetCount(); j++)
	{
		m_Name_field_charakteristics.SetCurSel(j);
		if (j == NumberFieldCharakteristics)
		{
			CurrentSelect = true;
			break;
		}
...
void EditFilesImpDlg::OnCircles()
{
...
void EditFilesImpDlg::OnPrimitives()
{
	// TODO: Add your control notification handler code here
	CWnd* pWnd = GetDlgItem(IDC_SELECT_TYPE_DENOTATIONS);
	pWnd->SetWindowText("Тип преобразуемых обозначений:");
...
void EditFilesImpDlg::OnImpCharakteristics()
{
	// TODO: Add your control notification handler code here
	if (m_Import_charakteristics.GetCheck())
	{
...
void EditFilesImpDlg::OnOpenSelectFile()
{
	// TODO: Add your control notification handler code here
	string s;
	ifstream in;
...
void EditFilesImpDlg::OnChangeFileName()
{
...
void EditFilesImpDlg::OnChangeSizeDenotations()
{
	// TODO: If this is a RICHEDIT control, the control will not
	// send this notification unless you override the CDialog::OnInitDialog()
	// function and call CRichEditCtrl().SetEventMask()
...
void EditFilesImpDlg::OnChangeSizeCharacteristics()
{
	// TODO: If this is a RICHEDIT control, the control will not
	// send this notification unless you override the CDialog::OnInitDialog()
	// function and call CRichEditCtrl().SetEventMask()
...
void EditFilesImpDlg::OnOK()
{
	...
	while (!FilesRS.IsEOF())
	{
		...
		if (FilesRS.m_Name_file == buffer && FilesRS.m_Id_profile == IdProfile && FilesRS.m_Name_file != fn)
		{
