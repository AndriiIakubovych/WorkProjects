...
BOOL EditFilesExpDlg::OnInitDialog() 
{
	...
	m_List.InsertColumn(0, "Имя файла для экспорта", LVCFMT_LEFT, rect.Width() / 3 - (winDC.GetDeviceCaps(LOGPIXELSY) == 96? 5: 6), 0);
	m_List.InsertColumn(1, "Вид графического объекта", LVCFMT_LEFT, rect.Width() / 3 - (winDC.GetDeviceCaps(LOGPIXELSY) == 96? 5: 6), 1);
	m_List.InsertColumn(2, "Тип графического объекта", LVCFMT_LEFT, rect.Width() / 3 - (winDC.GetDeviceCaps(LOGPIXELSY) == 96? 6: 7), 2);
	TplineRead rstl;
...
void EditFilesExpDlg::OnOK() 
{
	...
	while (!FilesRS.IsEOF())
	{
		...
		{
			RepeateFile = true;
			MessageBox("Файл для данного профиля с именем \"" + NameFile_str + "\" уже существует!", "Сообщение", MB_OK | MB_ICONINFORMATION);
			break;
		}
		FilesRS.MoveNext();
	}
